...
class Double485Endpoint(AbstractEndpoint):
    DEVFUNC_WRITE_DSETTINGS = 0x05
    DEVFUNC_READ_DSETTINGS = 0x04
    DEVFUNC_GET_ALL_SETTINGS = 0x10
    DEVPORT_SETTINGS = 0x04
    ...
    PACKET_TYPES_CURRENT_DATA = [0x01, 0x02]
    PACKET_TYPES_SETTINGS = [0x11]
    PACKET_TYPES_RESPONSE = [0x21]
    PACKET_TYPES_OSC = [0x31]

    PRIMARY_DEVICE = DanfossDevice.DEVICE_ID  # Only Danfoss device can be primary

    def __init__(self, dev_eui, api_id):
        self.devEui = dev_eui
        self.apiId = api_id
        self._devices = {}
